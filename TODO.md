...
- [x] Fix all-day task dates off by one day. TickTick stores all-day due dates as midnight UTC of the NEXT day (half-open interval). Without correction, "due Monday May 25" rendered as "May 26" after timezone conversion. New `all_day_date()` helper extracts the logical calendar date by staying in UTC and subtracting one day for end-boundary fields (due_date). Applied to markdown list rows, markdown detail view, JSON output (emits date-only YYYY-MM-DD for all-day tasks instead of misleading full ISO timestamps), and all server-side date filters (due_today, overdue, due_before, due_after) via `logical_due_date()` helper (2026-05-25)
