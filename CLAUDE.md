...
## Debugging TickTick V2 auth (when V2 tools fail)

When V2 tools fail (`search_tasks`, `list_tasks`, `get_status`, tags, folders, habits, focus, subtasks), **see the "Debugging checklist (when V2 is down)" in `docs/ARCHITECTURE.md` §4.** Short version: ask for the Railway logs first, run `ticktick_auth_status`, and remember `429` (throttle, wait) is not `401` (stale cookie, refresh).
